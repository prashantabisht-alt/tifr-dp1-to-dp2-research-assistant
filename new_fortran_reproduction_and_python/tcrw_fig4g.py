"""
TCRW Fig 4(g) — OBC complex-plane spectrum at fixed D_r, L = 10, varying ω
==========================================================================

Paper: Osat, Meyberg, Metson, Speck, arXiv:2602.12020, Fig 4(g).

Same 2×3 layout as 4(f) but the column axis is ω instead of D_r:
    ω ∈ {0.5, 0.7, 0.9}  (confirmed from paper column headers)
    D_r = 0.1            (not labelled in the panel; best guess from 4(e))
    L   = 10

Two coloring schemes (paper rows) reveal a different facet of Fig 4(a)'s
partition of boundary states:
...
def spot_checks(L: int = 10):
    ...
    # Paper parameters for 4(g): fixed D_r = 0.1, ω scanning.
    for omega in (0.5, 0.7, 0.9):
        e, w_to, w_ccw = obc_spectrum_with_weights(omega, 0.1, L)
        assert np.min(np.abs(e - 1.0)) < 1e-10
        assert np.max(np.abs(e)) <= 1.0 + 1e-12
        assert np.all(w_to + w_ccw <= 1.0 + 1e-12)
    print(f"  λ = 1 present; |λ| ≤ 1; w_to + w_ccw ≤ 1 at all three ω")

    # Extra ω ↔ 1 − ω symmetry check at fixed D_r (same as 4(e) test)
    e1, _, _ = obc_spectrum_with_weights(0.3, 0.1, L)
    e2, _, _ = obc_spectrum_with_weights(0.7, 0.1, L)
    d_re = float(np.max(np.abs(np.sort(e1.real) - np.sort(e2.real))))
    d_im = float(np.max(np.abs(np.sort(e1.imag) - np.sort(-e2.imag))))
    print(f"  ω=0.3 ↔ 0.7 symmetry: max|ΔRe|={d_re:.2e}, max|ΔIm+Im'|={d_im:.2e}")
    assert d_re < 1e-10 and d_im < 1e-10


# ---------------------------------------------------------------------------
# 6. Figure
# ---------------------------------------------------------------------------
def make_figure(L: int = 10, omega_list=(0.5, 0.7, 0.9), D_r: float = 0.1,
                savepath: str = "tcrw_fig4g_paper.png"):
    # Compute once per ω at fixed D_r
    data = []
    print(f"  computing 2x{len(omega_list)} panels  (D_r={D_r}, L={L})")
    t0 = time.time()
    for w in omega_list:
        e, w_to, w_ccw = obc_spectrum_with_weights(w, D_r, L)
        data.append((w, e, w_to, w_ccw))
    ...
    for col, (w, e, w_to, w_ccw) in enumerate(data):
        ...
        sc_b = ax_b.scatter(e.real, e.imag, c=w_ccw, cmap=cmap_ccw,
                            vmin=0, vmax=1, s=18, edgecolors="none", alpha=0.9)
        # Column title: ω value
        ax_t.set_title(rf"$\omega = {w}$", fontsize=11)
    ...
    fig.suptitle(r"(g)  OBC spectrum at $D_r = " + f"{D_r}" + r"$, $L = 10$",
                 fontsize=12, y=0.95)
...
if __name__ == "__main__":
    print("=" * 62)
    print(" TCRW Fig 4(g) — OBC complex-plane at D_r = 0.1, L = 10")
    print(" paper ω = 0.5, 0.7, 0.9 ; colorings: in-edge + CCW")
    print("=" * 62)

    print("\n[1] Author cross-check at L = 2")
    crosscheck_authors(0.5, 0.1, L=2)
    crosscheck_authors(0.7, 0.1, L=2)
    ...
    print("\n[3] Production figure")
    make_figure(L=10, omega_list=(0.5, 0.7, 0.9), D_r=0.1,
                savepath="tcrw_fig4g_paper.png")
