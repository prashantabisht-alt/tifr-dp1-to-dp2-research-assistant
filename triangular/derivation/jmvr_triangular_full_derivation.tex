...
\end{itemize}
The choice of $(a, b)$ does \emph{not} affect any algebra below; it
affects only the Cartesian visualisation. We keep $a, b$ symbolic.
...
With the sheared k-grid held correct, comparison vs KMC at three
parameter points:
\begin{center}\small
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{lrrr}
\toprule
$(\gamma, \epsilon, t)$ & buggy RMS & fixed RMS & ratio \\
\midrule
$(0.05,\, 0.10,\, 30)$ & $2.30\times10^{-4}$ & $3.12\times10^{-5}$ & $7\times$ \\
$(0.01,\, 0.15,\, 50)$ & $1.43\times10^{-4}$ & $3.43\times10^{-6}$ & $42\times$ \\
...
% ===============================================================
\section{Solving the dynamics}\label{sec:solving}

For each $\vk$:
\[
\big(\Ptilde_0, \ldots, \Ptilde_5\big)^T(\vk, t)
= e^{M(\vk)\, t}\, \big(\Ptilde_0, \ldots, \Ptilde_5\big)^T(\vk, 0).
\]
For walker at origin with uniformly random initial director:
$\Ptilde_m(\vk, 0) = \tfrac{1}{6}$. Computed via dense eigendecomposition
$M = V \Lambda V^{-1}$, then $e^{M t} = V e^{\Lambda t} V^{-1}$. Inverse
Fourier with (\ref{eq:ifft}) over the sheared grid gives $P(n_1, n_2, t)$.
...
\paragraph{1D ($K = 2$ directors).} Two states $\pm$ on a 1D lattice.
Forward rate $\tfrac{1}{2} + \epsilon$, backward $\tfrac{1}{2} - \epsilon$.
The $2\times 2$ matrix at momentum $k$ is
\[
M(k) = \begin{pmatrix} \cos k - 1 - \gamma + 2 i \epsilon \sin k & \gamma \\
\gamma & \cos k - 1 - \gamma - 2 i \epsilon \sin k \end{pmatrix}.
\]
(See draft Eq.~A6.) The chirality sign on the lower diagonal is
\emph{negative} -- exactly the conjugation pairing we derived for
triangular.

\paragraph{2D square ($K = 4$ directors).} Forward rate $\tfrac{1}{4} + \epsilon$,
backward $\tfrac{1}{4} - \epsilon$, two sideways at $\tfrac{1}{4}$. The
$4\times 4$ matrix has diagonal entries
\[
a = \tfrac{1}{2}(\cos k_x + \cos k_y) - 1 - \gamma + 2 i \epsilon \sin k_x,\quad
b = \tfrac{1}{2}(\cos k_x + \cos k_y) - 1 - \gamma + 2 i \epsilon \sin k_y,
\]
and the other two are $a^*, b^*$. (See draft Eq.~A17--A19.) Again the
chirality pairs by complex conjugation, and the $c_3$ sign error in
the triangular case is the analogue of getting $a^*$ vs $a$ confused on
the square lattice.

The triangular case adds one more independent direction (three pairs
instead of two), and the third chirality is the one where the sign got
flipped.
