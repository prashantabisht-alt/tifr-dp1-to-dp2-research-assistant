# Panel (b) — Kinetic Monte Carlo, P(x,y,t) on triangular lattice
# Usage:
#   gnuplot fig11_panel_b.gnu                  # PNG + PDF
#   gnuplot -e "mode='qt'"  fig11_panel_b.gnu  # interactive qt (mouse zoom)
#   gnuplot -e "mode='png'" fig11_panel_b.gnu  # PNG only
#   gnuplot -e "mode='pdf'" fig11_panel_b.gnu  # PDF only

if (!exists("mode")) mode = "both"

png_out = "fig11_panel_b.png"
pdf_out = "fig11_panel_b.pdf"
data_file = "outputs/fig11_final_hex_kmc_points.txt"
...
set xlabel "x"
set ylabel "y"
set title "(b) Kinetic Monte Carlo  N=100,000,000   gamma=0.01, epsilon=0.15, t=50"
...
if (mode eq "qt") {
    set terminal qt enhanced font "Helvetica,12" size 950,1000 \
        title "Fig 11 panel (b) — KMC  [right-drag: zoom | u: unzoom | a: full]"
    eval(do_plot)
    pause mouse close
}
